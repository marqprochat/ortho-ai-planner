...
const NovoPlanejamentoIA = () => {
  ...
  // Planning ID for updates (edit mode)
  const [planningId, setPlanningId] = useState<string | null>(() => {
    return (location.state as any)?.planningId || null;
  });
  ...
  // Reset form when "isNew" state is passed from navigation
  useEffect(() => {
    if (location.state && (location.state as any).isNew) {
      ...
      setImagePreviews([]);
      setMessages([]);
      // Clear state to avoid repeated resets
  ...
  useEffect(() => {
    if (user && !formData.nomeOrtodontista) {
      setFormData(prev => ({
        ...prev,
        nomeOrtodontista: user.name,
        croOrtodontista: user.cro || prev.croOrtodontista
      }));
    }
  }, [user]);
  ...
  const generateInitialDiagnosis = async () => {
    ...
    try {
      ...
      const systemPrompt = `Você é um Ortodontista Sênior com mais de 20 anos de experiência, especialista em Ortopedia Funcional dos Maxilares, Ortodontia Interceptativa e Ortodontia Fixa (Metálico/Estético), Autoligados e Alinhadores.
...
Mantenha todas as respostas CONCISAS e OBJETIVAS.`;
  ...
  const handleSaveDocumentation = async () => {
    ...
    try {
      ...
      if (planningId) {
        await patientService.updatePlanning(planningId, {
          status: 'DOCUMENTACAO_ENVIADA',
          structuredPlan: { ...formData, patientId: patientResult.patient.id },
        });
        toast.success("Documentação atualizada com sucesso!");
      } else {
        const newPlanning = await patientService.createPlanning({
      ...
      // Navigate to patient page to see saved documentation
      navigate(`/patients/${patientResult.patient.id}`);
    } catch (error: any) {
  ...
  const handleSelectOption = (option: string) => {
    ...
    navigate("/plano-de-tratamento", {
      state: {
        ...
        patientId,
        planningId, // Pass planningId for update
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <form onSubmit={(e) => { e.preventDefault(); generateInitialDiagnosis(); }} className="space-y-6" autoComplete="off">
            ...
            <div className="flex justify-end gap-3">
              <Button type="button" variant="outline" size="lg" className="gap-2" onClick={handleSaveDocumentation} disabled={isLoading} formNoValidate>
